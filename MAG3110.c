...
void MAG3110_GetXData(MAG3110_Data* sensorData) {
	sensorData->mag_X_msb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_X_MSB);
	sensorData->mag_X_lsb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_X_LSB);
...
void MAG3110_GetYData(MAG3110_Data* sensorData) {
	...
	I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_X_MSB);

	sensorData->mag_Y_msb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_Y_MSB);
	sensorData->mag_Y_lsb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_Y_LSB);
...
void MAG3110_GetZData(MAG3110_Data* sensorData) {
	...
	sensorData->mag_Z_msb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_Z_MSB);
	sensorData->mag_Z_lsb = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_OUT_Z_LSB);
...
{
	sensorData->die_temp = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_DIE_TEMP);
}

bool MAG3110_Is_X_Data_Ready()
{
	uint8_t temp = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_DR_STATUS);
	if (temp & MAG_XDR)
		return true;
	else
		return false;
...
{
	return I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_DR_STATUS);
}
...
void MAG3110_Set_Offest_Correction(uint16_t X_offset, uint16_t Y_offset, uint16_t Z_offset) {
	...
	// X Offset
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_X_MSB, (X_offset >> 8)&0xFF);
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_X_LSB, (X_offset)&0xFF);
	// Y Offset
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_Y_MSB, (Y_offset >> 8)&0xFF);
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_Y_LSB, (Y_offset)&0xFF);
	// Z Offset
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_Z_MSB, (Z_offset >> 8)&0xFF);
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_OFF_Z_LSB, (Z_offset)&0xFF);
...
{
	uint8_t temp;
	temp = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1);
	...
	if (temp & MAG_AC) {
		I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1, (temp & ~(1 << MAG_AC_BITSHIFT)));
		I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1, reg_value);
		I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1, (temp | (1 << MAG_AC_BITSHIFT)));
	} else {
		I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1, reg_value);
	}
...
{
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG2, reg_value);
}
...
void MAG3110_SetActive(bool enabled)
{
	...
	I2C_WriteToReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG1, temp);
}

void MAG3110_SetRaw(bool enabled) {
	uint8_t temp;
	temp = I2C_ReadFromReg(MAG_WHO_AM_I_VALUE, MAG_CTRL_REG2);
	if (enabled) {
